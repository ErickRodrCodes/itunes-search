...
const { searchSong } = require('../../dist/index');
const scope = nock('https://itunes.apple.com');
const mockData = require( '../mock-answers/searchSong.json');
...
describe('Search Song', () => {
  ...
  it("should not return results for a song called 'EXPERIMENTAL_TERM_OF_SEARCH'", async () => {
    scope.get('/search?term=EXPERIMENTAL_TERM_OF_SEARCH&limit=1&country=US&language=en&media=music')
      .reply(200, {
        "resultCount": 0,
        "results": []
      });
    const result = await searchSong('EXPERIMENTAL_TERM_OF_SEARCH');
  ...
  describe("it validates if you're passing valid optional parameters", () => {
    it("should throw exception when term is not a string, or empty string", async () => {
      ...
      await expect(searchSong(null, { limit: 3 }))
        .rejects
        .toHaveProperty(
          'message', exceptionMessage
        );
    ...
    it('should throw exception when language code is invalid', async () => {
      ...
      await expect(searchSong('Home by the sea', { language:'INVALID' }))
        .rejects
        .toHaveProperty(
          'message', exceptionMessage
        );
    ...
    it('should throw exception when country code is invalid', async () => {
      ...
      await expect(searchSong('Home by the sea', { country:'INVALID' }))
        .rejects
        .toHaveProperty(
          'message', exceptionMessage
        );
